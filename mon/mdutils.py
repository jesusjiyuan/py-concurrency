...
'''
SHA1加密
全称：安全哈希算法(Secure Hash Algorithm)主要适用于数字签名标准(Digital Signature Standard DSS)里面定义的数字签名算法(Digital Signature Algorithm DSA)，
...
实现原理是用公开函数和密钥产生一个固定长度的值作为认证标识，用这个标识鉴别消息的完整性。使用一个密钥生成一个固定大小的小数据块，即 MAC，并将其加入到消息中，然后传输。接收方利用与发送方共享的密钥进行鉴别认证等。Python代码：
'''
def test_hmac():
    import hmac
    import hashlib
...
'''
DES加密
全称：数据加密标准(Data Encryption Standard)，属于对称加密算法。DES是一个分组加密算法，典型的DES以64位为分组对数据加密，加密和解密用的是同一个算法。
...
def test_des():
    ...
    # 8位密钥
    #secret_str = des_encrypt('X4&dwMDz', 'Tengxunemail@123.jiyuan.')
    secret_str = des_encrypt('X4&dwMDz', 'mysql.')
    secret_str1 = des_encrypt('X4&dwMDz', 'weWE12!@')
    print(secret_str)
    print(secret_str1)
    print(secret_str.decode('utf-8'))
    print(secret_str.decode('utf-8').encode("utf-8"))
    #clear_str = des_decrypt('X4&dwMDz', secret_str)
    clear_str = des_decrypt('X4&dwMDz', b'5ddee105d7e9e24f74cfd827ed8a7547e770b13a6d23f3ad1533e0c8586ff07f')
...
'''
ECC加密
全称：椭圆曲线加密(Elliptic Curve Cryptography)，ECC加密算法是一种公钥加密技术，以椭圆曲线理论为基础。利用有限域上椭圆曲线的点构成的Abel群离散对数难解性，
实现加密、解密和数字签名。将椭圆曲线中的加法运算与离散对数中的模乘运算相对应，就可以建立基于椭圆曲线的对应密码体制。Python代码：
'''
def test_ecc():
    # -*- coding:utf-8 *-
    # author: DYBOY
