...
class LinkMonResultMapper:
    ...
    def list(link: LinkMonResult) -> list:
        ...
        conn = dbhepler.connect()
        cursor = conn.cursor()
        sql="select id,purl,url,picPath,status,createTime,updateTime,delFlag,matchPicPath,batchNo,ocrStr,ocrRaw from mon_link_result where delFlag = '0'"
        ...
        res = cursor.fetchall()
        for r in res:
            linktmp = LinkMonResult(r[0],r[1],r[2],r[3])
...
class MonVideoResultMapper:
    ...
    def update(cls,ved: MonVideoResult):
        conn = dbhepler.connect()
        cursor = conn.cursor()
        sql = "update mon_video_result set raw='{raw}',status='{status}',vedioDuration='{vedioDuration}' where id = '{id}'" \
            .format(id=ved.id,raw=ved.raw,status=ved.status,vedioDuration=ved.vedioDuration)
        #print(sql)
...
class MonVideoResultExtMapper:
    ...
    def update(vedio: MonVideoResultExt):
        conn = dbhepler.connect()
        cursor = conn.cursor()
        sql = "update mon_video_result_ext set kwords = kwords+',{kwds}' where vId='{vId}' and starts='{}' and ends='{}'"\
