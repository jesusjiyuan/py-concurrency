...
FILE_ROOT = Path(r"C:\Users\dell\Desktop")

# 原始数据集
IMAGE_SET_ROOT = FILE_ROOT.joinpath(r"VOC\ImageSets\Main")  # 图片区分文件的路径
IMAGE_PATH = FILE_ROOT.joinpath(r"VOC\JPEGImages")  # 图片的位置
ANNOTATIONS_PATH = FILE_ROOT.joinpath(r"VOC\Annotations")  # 数据集标签文件的位置
LABELS_ROOT = FILE_ROOT.joinpath(r"VOC\Labels")  # 进行归一化之后的标签位置
...
def save_label_file(img_jpg_file_name, size, img_box):
    ...
    with open(save_file_name, "a+") as f:
        for box in img_box:
            if box[0] == 'reflective_clothes':  # 数据集 xml 中的 person 指的是头
                cls_num = 0
            elif box[0] == 'other_clothes':
                cls_num = 1
            else:
                continue
